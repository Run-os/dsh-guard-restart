...
// dsh-guard-restart-asset: guard-cli.mjs v3
//
...
function readGuardConfig() {
    let cfg = null;
    try {
        cfg = JSON.parse(fs.readFileSync(guardConfigPath(), 'utf8'));
    }
    catch { /* defaults */ }
...
function listProfilePlugins(profile) {
    ...
    return names.map((name) => {
        const entryId = resolveEntryIdHint(profile, name) ?? '';
        const block = entryId ? patch.blocks.find((b) => b.id === entryId) : undefined;
        return {
            name,
            entryId: entryId || resolveEntryId(profile, name),
