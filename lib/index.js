...
export function apply(ctx, config) {
    ...
    // （实验性，默认关闭）。v0.4.0 曾默认开启：把 footerActions 槽容器改
    // column 会把位于第二行的 auto-memory 按钮挤出可视区（实测：移除该
    // class 后按钮即恢复），故默认改回 false。其余卡片内容（systemd /
    ...
    ctx.inject(['settings'], (sctx) => {
        try {
            ...
            sctx.settings.register(name, z.object({
                footerStack: z.boolean().default(false),
            }), { base: { footerStack: false } });
